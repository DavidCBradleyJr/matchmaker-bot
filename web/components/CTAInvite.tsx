...
const cid   = process.env.NEXT_PUBLIC_DISCORD_CLIENT_ID; // from GH Secrets
...
const inviteUrl = cid ? `https://discord.com/api/oauth2/authorize?client_id=${cid}&permissions=${perms}&scope=${scope}` : null;
...
export default function CTAInvite() {
  return (
    <div className="inline-flex items-center gap-3">
      ...
      {env === "staging" && (
        <span className="rounded-md border border-yellow-400/30 bg-yellow-500/10 px-2 py-1 text-xs text-yellow-200">STAGING</span>
      )}
    </div>
  );
}